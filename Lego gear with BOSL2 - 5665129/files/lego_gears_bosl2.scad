...
for (s = [0.5:1:3.5]) translate([s*s*8,0,0]) rotate([0,0,22.5*(s-0.5)]) lego_gear(s,flat_surface=true,solid=true);

color("violet")left(0) for (s = [1:1:2]) translate([-s*s*8-2,0,0]) rotate([0,0,22.5*s/2]) lego_gear(s, flat_surface=false);
...
module lego_gear(s, flat_surface=true, central_axle=true, solid=false, stud_hole_tolerance=0.00, axle_hole_tolerance=0.05) {
    ...
    difference() {
        ...
        union () {
            ...
            translate([0,0, 1.93]) spur_gear(mod=1, teeth=16*s, thickness=4, pressure_angle=20,anchor=BOT); //BOSL2 !!
